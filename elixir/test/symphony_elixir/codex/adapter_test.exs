defmodule SymphonyElixir.Codex.AdapterTest do
  ...
  test "app server passes explicit turn sandbox policies through unchanged" do
    ...
    try do
      ...
      Enum.each(policy_cases, fn configured_policy ->
        ...
        assert {:ok, _result} = Adapter.run(workspace, "Validate supported turn policy", issue)
  ...
  test "app server marks request-for-input events as a hard failure" do
    ...
    try do
      ...
      assert {:error, {:turn_input_required, payload}} =
               Adapter.run(workspace, "Needs input", issue)
  ...
  test "app server fails when command execution approval is required under safer defaults" do
    ...
    try do
      ...
      assert {:error, {:approval_required, payload}} =
               Adapter.run(workspace, "Handle approval request", issue)
  ...
  test "app server auto-approves command execution approval requests when approval policy is never" do
    ...
    try do
      ...
      assert {:ok, _result} = Adapter.run(workspace, "Handle approval request", issue)
  ...
  test "app server auto-approves MCP tool approval prompts when approval policy is never" do
    ...
    try do
      ...
      assert {:ok, _result} = Adapter.run(workspace, "Handle tool approval prompt", issue)
  ...
  test "app server sends a generic non-interactive answer for freeform tool input prompts" do
    ...
    try do
      ...
      assert {:ok, _result} =
               Adapter.run(workspace, "Handle generic tool input", issue, on_message: on_message)
  ...
  test "app server sends a generic non-interactive answer for option-based tool input prompts" do
    ...
    try do
      ...
      assert {:ok, _result} =
               Adapter.run(workspace, "Handle option based tool input", issue)
  ...
  test "app server rejects unsupported dynamic tool calls without stalling" do
    ...
    try do
      ...
      assert {:ok, _result} = Adapter.run(workspace, "Reject unsupported tool calls", issue)
  ...
  test "app server captures codex side output and logs it through Logger" do
    ...
    try do
      ...
      log =
        capture_log(fn ->
          assert {:ok, _result} =
                   Adapter.run(workspace, "Capture stderr log", issue, on_message: on_message)
        end)
